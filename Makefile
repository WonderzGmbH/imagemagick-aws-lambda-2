...
build/output.yaml: template.yaml build/layer.zip
	aws --profile=wonderz-clone --region=us-east-1 cloudformation package --template $< --s3-bucket $(DEPLOYMENT_BUCKET) --output-template-file $@

deploy: build/output.yaml
	aws --profile=wonderz-clone --region=us-east-1  cloudformation deploy --template $< --stack-name $(STACK_NAME)
	aws --profile=wonderz-clone --region=us-east-1  cloudformation describe-stacks --stack-name $(STACK_NAME) --query Stacks[].Outputs --output table
